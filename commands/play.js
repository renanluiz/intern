...
module.exports = {
  ...
  cooldown: 3,
  aliases: ["p"],
  description: "Toca audios do Youtube ou Soundcloud",
  async execute(message, args) {
    ...
    if (!args.length)
      return message
        .reply(`Digite: ${message.client.prefix}play < URL do YouTube | Nome do vídeo | URL do Soundcloud >`)
        .catch(console.error);

    const permissions = channel.permissionsFor(message.client.user);
    if (!permissions.has("CONNECT"))
      return message.reply("Não foi possível conectar ao canal por falta de permissões");
    if (!permissions.has("SPEAK"))
      return message.reply("Eu não consigo tocar nesse canal de voz, por favor garanta que eu tenha as permissões necessárias!");
      ...
      try {
        https.get(url, function (res) {
          if (res.statusCode == "302") {
            ...
            return message.reply("Nenhum conteúdo foi encontrado nessa URL.").catch(console.error);
          }
        });
      } catch (error) {
        console.error(error);
